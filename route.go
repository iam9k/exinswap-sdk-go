...
func EncodeRoutes(ids []string) string {
	s := ""
	for _, id := range ids {
		s = s + "_" + id
	}
...
func (g Graph) AddPair(pair *Pair) {
	g.Add(pair.Asset0.UUID, pair.Asset1.UUID, pair)
	g.Add(pair.Asset1.UUID, pair.Asset0.UUID, pair)
}
...
func (n *node) Contain(id string) bool {
...
func (n *node) route(g Graph, fillAsset string, best *node) {
	...
	for fill, pair := range g[n.FillAssetID] {
		if n.Contain(pair.LpAsset.RouteId) {
			continue
		}
